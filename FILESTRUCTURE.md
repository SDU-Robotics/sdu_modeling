...
* Python source files:
  * The Python package source files are located in the `sdu_modeling` directory.
  * The file `sdu_modeling.__main__.py` defines a command line interface that
    is accessible both as the command `sdu_modeling` and via
    `python -m sdu_modeling`.
  * `tests/test_cli.py` contains rudimentary testing of this CLI.
  * `tests/test_sdu_modeling.py` contains the unit tests for the package.
